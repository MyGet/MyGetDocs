...
* Universal Apps support (Visual Studio 2013 Update 3)
* PCL support
* Windows Azure SDK 1.7, 1.8, 2.0, 2.1, 2.2, 2.3, 2.4
...
Running ```GitVersion /output buildserver``` will cause MyGet Build Services to set the current ```%PackageVersion%``` value to the NuGet-compatible SemVer generated by GitVersion. Additionally, GitVersion will create the following environment variables. The variables are explained and demonstrated in the [GitVersion documentation](https://github.com/Particular/GitVersion/wiki/Command-Line-Tool#consume-gitversion-from-build-scripts).
