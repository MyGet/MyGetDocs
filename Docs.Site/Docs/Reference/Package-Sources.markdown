...
* authentication against upstream, private MyGet feeds happens automatically (see [Package Source Credentials](/docs/reference/package-sources#Package_Source_Credentials))
...
Once package source mirroring is enabled, we can consume our MyGet feed from Visual Studio which will also list upstream packages. For example, the example _acmecompany_ feed only lists one package:
...
![Package Source Options](Images/add-package-source-options.png)
